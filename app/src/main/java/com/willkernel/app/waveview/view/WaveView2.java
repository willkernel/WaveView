...
public class WaveView2 extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private static final int backgroundColor=Color.parseColor("#4d8cd3");
...
//            holder.removeCallback(this);//on pause() 后会移除view
        }
    }

    private void doDraw(Canvas canvas, long millsPassed) {
        if (samplingX == null) {
        ...
        for (int i = 0; i <= SAMPLING_SIZE; i++) {
            ...
            //中间那条路径的振幅是上下的1/5
            centerPath.lineTo(x, centerHeight + curV / 4F);
        ...
        for (int i = 2; i < crestAndCrossCount; i += 2) {
            ...
            mPaint.setShader(new LinearGradient(0, centerHeight + crestY, 0, centerHeight - crestY, Color.parseColor("#86add8"), Color.parseColor("#86add8"), Shader.TileMode.CLAMP));
    ...
    private double calcValue(float x, float offset) {
        ...
        double dampingFunc = Math.pow(4/ (4 + Math.pow(x, 4)), 2);
